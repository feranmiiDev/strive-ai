  ...
  const handleSaveHistory = async () => {
    ...
    try {
      ...
      const recordsPayload = results.results.map((res, i) => ({
        ...
        midsemester_score: courses[i]?.midsemester_score || 0,
        assignment_score: courses[i]?.assignment_score || 0.0,
        quiz_score: courses[i]?.quiz_score || 0.0,
        ca_score: ((courses[i]?.midsemester_score || 0) + ((courses[i]?.attendance || 0) * 0.1) + (courses[i]?.assignment_score || 0) + (courses[i]?.quiz_score || 0)),
              ...
              <div className="modal-card-body docs-modal-body">
                ...
                <section className="docs-section">
                  ...
                  <ol className="docs-steps-list">
                    ...
                    <li><strong>Plan Curriculum</strong>: Under "My Courses", load your credit unit weights, midsemester scores, and attendance rates.</li>
                ...
                {courses.map((course, index) => (
                  <div key={index} className="curriculum-card-expanded">
                    ...
                    <div className="grid-inputs">
                      ...
                      <div className="form-group">
                        <label>Midsemester (0-15)</label>
                        <input
                          type="number"
                          min="0"
                          max="15"
                          value={course.midsemester_score}
                          onChange={(e) => handleInputChange(index, 'midsemester_score', e.target.value)}
                          required
                        />
                      </div>
                  ...
                  return (
                    <div key={idx} className="simulations-full-course-card">
                      ...
                      <div className="sim-sliders-large-grid">
                        ...
                        <div className="sim-slider-row">
                          <div className="sim-slider-lbl-row">
                            <span className="sim-slider-title">Midsemester Exam (0-15)</span>
                            <span className="sim-slider-val-badge">{course.midsemester_score} pts</span>
                          </div>
                          <input 
                            type="range" min="0" max="15" step="0.5"
                            value={course.midsemester_score}
                            onChange={(e) => handleSliderChange(idx, 'midsemester_score', e.target.value)}
                            className="emerald-slider"
                          />
                        </div>
